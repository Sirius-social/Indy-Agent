...
class Connection(WireMessageFeature, metaclass=FeatureMeta):
    ...
    async def handle(cls, agent_name: str, wire_message: bytes, my_label: str = None, my_endpoint: str = None) -> bool:
        ...
        if message.type == cls.REQUEST:
            ...
            return True
        elif message.type == Connection.RESPONSE:
    ...
    @classmethod
    async def receive_invite_message(cls, msg: Message, agent_name: str, pass_phrase: str, my_label: str,
                                     my_endpoint: str, ttl: int) -> str:
    ...
    class InviterStateMachine(BaseStateMachine, metaclass=InvokableStateMachineMeta):
        ...
        async def handle(self, content_type, data):
            try:
                if content_type == Connection.MESSAGE_CONTENT_TYPE:
                    kwargs = json.loads(data)
                    msg = Message(**kwargs)
                elif content_type in WIRED_CONTENT_TYPES:
                    msg = await Connection.unpack_agent_message(data, self.get_wallet())
                else:
                ...
                elif msg.type == Connection.PROBLEM_REPORT:
        ...
        async def __receive_connection_request(self, msg: Message):
            if self.status == DIDExchangeStatus.Invited:
                success, err_msg = await Connection.validate_common_message_blocks(msg)
                if success:
                    try:
                        Connection.Request.validate(msg)
                    except Exception as e:
                        logging.exception('Error while parsing message %s with error %s' % (msg.as_json(), str(e)))
                        vk, endpoint = BasicMessage.extract_verkey_endpoint(msg, Connection.CONNECTION)
                        if None in (vk, endpoint):
                            ...
                            err_msg = Connection.build_problem_report_for_connections(
                                Connection.REQUEST_NOT_ACCEPTED,
                                str(e),
                                thread_id=msg.id
                            )
                            await Connection.send_message_to_endpoint_and_key(vk, endpoint, err_msg, self.get_wallet())
                            await self.__log('Send report problem', err_msg.to_dict())
                    else:
                        try:
                            connection_key = msg.context['to_key']
                            label = msg['label']
                            their_did, their_vk, their_endpoint = BasicMessage.extract_their_info(msg,
                                                                                                  Connection.CONNECTION)
                    ...
                    their_did = msg.context.get('from_did')
                    if their_did:
                        await Connection.send_message_to_agent(their_did, err_msg, self.get_wallet())
                        await self.__log('Send report problem', err_msg.to_dict())
        ...
        async def __send_connection_response(self, their_did: str):
            ...
            response_msg = Connection.Response.build(pairwise_meta['req_id'], my_did, my_vk, self.endpoint)
            # Apply signature to connection field, sign it with the key used in the invitation and request
            response_msg['connection~sig'] = \
                await Connection.sign_agent_message_field(
                    self.get_wallet(),
                    response_msg['connection'],
                    pairwise_meta["connection_key"]
                )
            del response_msg['connection']
            await Connection.send_message_to_agent(their_did, response_msg, self.get_wallet())
            return response_msg
    ...
    class InviteeStateMachine(BaseStateMachine, metaclass=InvokableStateMachineMeta):
        ...
        async def handle(self, content_type, data):
            try:
                if content_type == Connection.MESSAGE_CONTENT_TYPE:
                    ...
                    raise RuntimeError('Unknown content_type "%s"' % content_type)
                await self.__log('Receive', msg.to_dict())
                if msg.type == Connection.INVITE:
                    await self.__receive_invitation(msg)
                elif msg.type == Connection.RESPONSE:
        ...
        async def __send_connection_request(self, invitation: Message):
            ...
            request = Connection.Request.build(self.label, my_did, my_vk, self.endpoint)
            try:
        ...
        async def __receive_connection_response(self, msg: Message):
            if self.status == DIDExchangeStatus.Requested:
                ...
                if success:
                    ...
                    if my_did is None:
                        msg[Connection.CONNECTION], sig_verified = \
                        ...
                        if not sig_verified:
                            ...
                            if None in (vk, endpoint):
                                ...
                                await Connection.send_message_to_endpoint_and_key(vk, endpoint, err_msg)
                                await self.__log('Send report problem', err_msg.to_dict())
                    else:
                        ...
                        msg[Connection.CONNECTION], sig_verified = \
                            await Connection.unpack_and_verify_signed_agent_message_field(msg['connection~sig'])
                        their_did, their_vk, their_endpoint = BasicMessage.extract_their_info(msg,
                                                                                              Connection.CONNECTION)
                        ...
                        if their_vk != msg_vk:
                            err_msg = \
                                Connection.build_problem_report_for_connections(
                                    Connection.KEY_ERROR,
                                    "Key provided in response does not match expected key",
                                    thread_id=msg.id
                                )
                            verkey, endpoint = BasicMessage.extract_verkey_endpoint(msg, Connection.CONNECTION)
                            logging.error("Key provided in response does not match expected key")
                            await Connection.send_message_to_endpoint_and_key(verkey, endpoint, err_msg)
                            await self.__log('Send report problem', err_msg.to_dict())
                            return
